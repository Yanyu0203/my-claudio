...
async function handleClientMessage(msg, sender) {
  ...
  switch (type) {
    ...
    case 'chat': {
      ...
      // 其它一律走 chatReply：让 DJ 真正回话
      console.log(`[chat] 用户说: "${text.slice(0, 40)}", 调大脑回话...`);
      ...
      try {
        ...
        sender.send('dj_say', { text: '（大脑短路了一下，再说一遍？）' });
        break;
      }
      console.log(
        `[chat] DJ 回 (${((Date.now() - tChat) / 1000).toFixed(1)}s): "${r.reply.slice(0, 50)}", regen=${r.should_regen}, replace=${r.replace_now}, delta="${r.taste_delta}"`
      );

      // 1. 回话先呈现
      store.recordMessage({ role: 'dj', text: r.reply });
      sender.send('dj_say', { text: r.reply });

...
> 💡 不喜欢可以重跑。或者改 `vox/scripts/bootstrap-taste.js` 里的 `TARGET_PLAYLIST_NAMES` 换成你自己的歌单名。
...
| `bootstrap:taste` 报"找不到目标歌单" | 你账号里没有「这段爱听」「好歌」这两个名字 | 改 `vox/scripts/bootstrap-taste.js` 里 `TARGET_PLAYLIST_NAMES` |
